...
def falcon_dashboard_pdf_layout():
    ...
    with st.sidebar:
        ...
        # Sub-option for the B.1 High Detections chart (indented under Detection and Severity)
        # Critical Detections in B.1 always show; the High Detections bar chart is opt-in (hidden by default).
        include_high_detections = False
        if include_detection_analysis:
            include_high_detections = st.checkbox(
                "    ↳ High Detections chart (B.1)",
                value=False,
                help="Show the B.1 High Detections bar chart beside the Critical boxes. Off by default; Critical Detections always show."
            )
...
> Counted directly from the `Resolution` column in the detection file, after excluding in-progress rows (`Status = in_progress`)
    ...
    if include_detection_analysis:
        ...
        if 'critical_high_overview' in detection_data:
            ...
            # Layout: side-by-side (Critical | High) only when the High chart is enabled;
            # otherwise Critical spans the full width. High Detections is opt-in (default off).
            if include_high_detections:
                col_critical, col_high = st.columns(2)
            else:
                col_critical = st.container()
                col_high = None
            ...
            # RIGHT COLUMN: High Detections (independent bar chart) - opt-in, hidden by default
            if include_high_detections and col_high is not None:
                with col_high:
